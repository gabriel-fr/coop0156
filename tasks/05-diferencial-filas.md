...
- [ ] No `contratar`: em vez de atualizar direto para `contratado`, atualizar o status
  para `StatusAnalise::PROCESSANDO_CONTRATACAO` e disparar
  `ProcessarContratacaoJob::dispatch($analise->id)`.
- [ ] Implementar `ProcessarContratacaoJob::handle()`:
...
- [ ] Rodar as migrations da tabela `jobs` (já existe migration padrão do Laravel,
  `0001_01_01_000002_create_jobs_table.php` — conferir que já rodou).
- [ ] Rodar o worker em terminal separado: `./vendor/bin/sail artisan queue:work` (ou
  `php artisan queue:work` na Opção B).
- [ ] Ajustar teste de contratação em `AnaliseCreditoTest` (ver [04](04-testes-automatizados.md))
...
- [ ] Frontend: a tela `analise.blade.php` já tem um card pronto para esse cenário
